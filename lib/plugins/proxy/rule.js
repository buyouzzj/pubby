...
const isMatch = utils.isMatch;
const getParams = utils.getParams;
const getRes = utils.getRes;

const join = require('path').join;
const readFileSync = require('fs').readFileSync;
const parseUrl = require('url').parse;
const isPlainObject = require('is-plain-object');
const getQuery = require('qs').parse;
  ...
  return {
    ...
    dealLocalResponse(_req, reqBody, callback) {
      ...
      return batchMatch(req, getProxyConfig(), (val, pattern) => {
        ...
        if (typeof val === 'function') {
          console.log(`${req.method} ${req.url} matches ${pattern}, respond with custom function`);
        ...
        if (typeof val === 'string' && !isRemote(val)) {
          console.log(`${req.method} ${req.url} matches ${pattern}, respond with local file`);
        ...
        if (isPlainObject(val) || Array.isArray(val)) {
          console.log(`${req.method} ${req.url} matches ${pattern}, respond with object or array`);
    ...
    */
    ...
    replaceRequestOption(req, option) {
      ...
      batchMatch(req, proxyConfig, (val, pattern) => {
        if (isRemote(val)) {
          // console.log(`${req.method} ${req.url} matches ${pattern}, forward to ${val}`);
          isModified = true;
          setOption(val);
          return true;
        }
